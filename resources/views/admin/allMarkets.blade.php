@extends('admin.index')
@section('title', 'Venoola | All Markets')
@section('style')
    <style>
        .rowStyle{
            font-weight: normal;
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 col-sm-6 col-xs-12 lft padding-bb">
                        <div class=" nw-pd col-md-1 col-md-offset-6">
                            <button  class="btn btn-primary border-r"  style="background-color: #4b646f" onclick="applySearch({{route('adminVendor.index')}})">Search</button>
            ...
            <table  class="table table-striped table-hover table-bordered " >
                ...
                @foreach($markets as $market)
                    <tr>
                        ...
                        <th class="rowStyle">{{$market->vendor->name}}</th>
                        <th class="rowStyle">{{$market->vendor->phone}}</th>
                        ...
                        <th class="rowStyle">{{$market->active}}</th>
                        <th class="rowStyle"><button type="button" style="background-color: #4b646f" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModalupdate-{{$market->id}}"><i class="fa fa-edit"></i> Edit Status</button>  <button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#myModaldelete-{{$market->id}}"><i class="fa fa-trash"></i> Delete</button>  <a href="{{route('marketProduct.index',$market->id)}}" style="background-color: #26b1b0" class="btn btn-xs btn-info" ><i class="fa fa-eye"></i> Show product</a></th>
                    </tr>
                @endforeach
                </tbody>
            </table>
                ...
                <div class="modal-dialog modal-sm "  >
                    ...
                    <div class="modal-content "style="text-align: center;">
                        ...
                        <h4 class="deleteModel">Are you sure you want to delete this</h4>
                            <form method="POST" id="myformdelete" action="{{route('deleteMarket.delete', $market->id)}}">
                                @csrf
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="POST" id="myformupdate" action="{{route('updateMarketStatus', $market->id)}}">
                                @csrf
                                <div class="form-group">
                                    <label for="pay">Status</label>
                                    <select name="active"  class="form-control border-r">
                                        <option value="new"{{$market->active == 'new' ? 'selected' : ''}}>New</option>
                                        <option value="active"{{$market->active == 'active' ? 'selected' : ''}}>Active</option>
                                    </select>
                                </div>
